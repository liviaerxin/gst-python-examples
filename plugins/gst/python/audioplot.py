...
'''
[Implementing an audio plotter](https://mathieuduponchelle.github.io/2018-02-15-Python-Elements-2.html)
'''
import gi
import numpy as np
...
class AudioPlotFilter(GstBase.BaseTransform):
    ...
    def do_transform_caps(self, direction, caps, filter_):
        print(f"do_transform_caps, direction: {direction}, caps:{caps}")
    ...
    def do_fixate_caps(self, direction, caps, othercaps):
        print(f"do_fixate_caps, direction:{direction}, caps:{caps}, othercaps:{othercaps}, othercaps.fixate(): {othercaps.fixate()}")
        if direction == Gst.PadDirection.SRC:
    ...
    def do_set_caps(self, icaps, ocaps):
        print(f"do_set_caps, icaps:{icaps}, ocaps:{ocaps}")
